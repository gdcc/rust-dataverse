//! Unix socket utilities for handling streaming data connections.
//!
//! This module provides functionality for managing Unix socket connections
...
/// Sets up a finish listener for Unix socket graceful shutdown.
///
/// Creates a secondary Unix socket with a ".finish" suffix that can be
/// used to signal when the upload should be completed and all connections
/// should be closed gracefully.
///
/// The finish socket allows external processes to signal completion by
/// connecting to it, which triggers the graceful shutdown sequence.
///
/// # Arguments
/// * `listener` - The main Unix listener to derive the finish socket path from.
///
/// # Returns
/// A `Result` containing the finish `UnixListener` or an error.
///
/// # Errors
/// Returns an error if:
/// - The main listener's socket path cannot be determined
/// - The finish socket cannot be bound
/// - File system operations fail when removing existing socket files
pub(crate) fn setup_finish_listener(
    listener: &UnixListener,
) -> Result<UnixListener, Box<dyn Error>> {
    ...
    // Clean up any existing finish socket file
    if Path::new(&finish_address).exists() {
        if let Err(e) = std::fs::remove_file(&finish_address) {
            eprintln!("Warning: Failed to remove existing socket file: {}", e);
        }
    }

    Ok(UnixListener::bind(finish_address)?)
}
